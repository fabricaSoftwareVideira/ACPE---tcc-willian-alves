...
const DashboardPage = () => {
  ...
  const uploadFile = async (file: File, fileName: string, userId: string) => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      if (!values.file || !session) {
        throw new Error("Nenhum arquivo ou sess o v lida foi encontrada.");
      }

      const hash = await calculatePdfHash(values.file);
      ...
      if (!querySnapshot.empty) {
        ...
        const pathFile = await uploadFile(values.file, fileName, session?.user.id);
                  ...
                  <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
                    ...
                    <DrawerFooter>
                      <Button type="submit" className="w-full" disabled={loading}>
                        {loading ? <LoadingSpinner className="bg-dark" /> : "Enviar"}
                      </Button>
                      <DrawerClose>
